    ...
    public class RabbitMQEndActiveCert : BackgroundService
    {
        ...
        private Task startrabbitMQ()
        {
            ...
            consumer.Received += (model, ea) =>
            {
                ...
                ObligatoryService obligatoryService = _context.ObligatoryServiceDB.Where(x=>x.UserID==CurrentID).OrderByDescending(x => x.DateOfEnd).FirstOrDefault();
                if (obligatoryService != null)
                {
                    obligatoryService.DateOfEnd = DateTime.Now;
                    _context.Update(obligatoryService);
                }
            };
